...
if 'Au restaurant' in options :
  ...
  Map.add_markers_from_xy(df_resto, x="longitude", y="latitude", popup=["Nom", "Adresse"], icon_shape = "circle", border_color='black', border_width=1, icon = "", layer_name="Au Restaurant", background_color = "#7a7a99")
if "Magasins d’alimentation" in options :
  df_alim = pd.read_csv("pages/aigo_alimentation.csv")
  Map.add_markers_from_xy(df_alim, x="longitude", y="latitude", popup=["Nom", "Adresse"], icon_shape = "circle", border_color='black', border_width=1, icon = "", layer_name="Magasins d’alimentation", background_color = "#997A7A")
